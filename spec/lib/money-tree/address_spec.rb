...
describe MoneyTree::Address do
  ...
  context "testnet3" do
    ...
    it "returns a bitcoin testnet address" do
  ...
  context "xpchain" do
    before do
      @address = MoneyTree::Address.new network: :xpchain
    end

    it "returns a xpchain main address" do
      expect(%w(X)).to include(@address.to_s(network: :xpchain)[0])
    end
  end

  context "xpchain_testnet" do
    before do
      @address = MoneyTree::Address.new network: :xpchain_testnet
    end

    it "returns a xpchain testnet address" do
      expect(%w(x)).to include(@address.to_s(network: :xpchain_testnet)[0])
    end
  end
end
